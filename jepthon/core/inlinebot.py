...
def main_menu():
    text = f"**- [سـورس ريبـــثون](https://t.me/Repthon)\
        \n المسـاعد\
        \n\
        \n◽ ريبـــثون لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس ريبـــثون 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس ريبـــثون 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس ريبـــثون 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت ريبـــثون 🧸♥](https://t.me/Repthon)**\
            \n\
            \n❤ ريبـــثون هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
